...
__global__ void Add(unsigned int *Da,int high,int width)
{
	...
	__shared__ unsigned int share[1024]; // 同一個block的threads來共享一個shared memory
	
	share[tx] = Da[id]; // copy到shared memory
	__syncthreads(); // 同步shared memory裡的threads 
	...
	Da[id] = share[0]; // copy回global memory
}
