		...
		while (file >> str)//讀取紀錄，若讀取至檔案結尾則回傳0，跳出while迴圈再次讀取 
        {
			...
			for (int i = 1; i < str.length(); i++)
            {
                ...
                {		            	
                	if (str.at(i - j) == str.at(i + j))//abcba 有中心  
                    {
                        ...
                        center = 0;
						count1[i] += 2;
                        arr1[i] = str.substr(i - j, j * 2 + 1);//bcb →abcba 找尋部分字串s1.substr(起始位置，字串長度) 
                    }
                    else
                    	break;
                }
            ...
            for (int i = 0; i < str.length(); i++)
            {
                for (int j = 0; i - j >= 0 && i + j + 1 < str.length(); j++)
                {
                    ...
                    {
                        count2[i] += 2;
                        arr2[i] = str.substr(i - j, j * 2 + 2);//bb →abba 部分字串s1.substr(起始位置，字串長度) 
                    }
                    else
                    	break;
                }
